<template>
  ...
  <Teleport to="body">
    ...
    >
      <div class="t-popup__content">
        ...
        <template v-else>
          <!-- 文档信息头部 -->
          <div class="tip-doc-header">
            <div class="tip-doc-info">
              <div v-if="floatPopup.knowledgeTitle" class="tip-doc-title">{{ floatPopup.knowledgeTitle }}</div>
              <div v-if="floatPopup.knowledgeBaseName" class="tip-doc-meta">
                <span class="tip-doc-kbname">{{ floatPopup.knowledgeBaseName }}</span>
                <template v-if="floatPopup.tagName">
                  <span class="tip-doc-sep"> · </span>
                  <span class="tip-doc-tag">{{ floatPopup.tagName }}</span>
                </template>
              </div>
            </div>
            <button
              v-if="floatPopup.knowledgeId"
              class="tip-preview-btn"
              :title="$t('chat.previewDocument')"
              @click.stop="openDocumentPreview"
            >
  ...
  <DocumentPreviewDrawer
    v-model:visible="docPreviewDrawer.visible"
    :knowledgeId="docPreviewDrawer.knowledgeId"
    :knowledgeTitle="docPreviewDrawer.knowledgeTitle"
    :knowledgeBaseName="docPreviewDrawer.knowledgeBaseName"
    :tagName="docPreviewDrawer.tagName"
    :fileType="docPreviewDrawer.fileType"
  />
...
const openDocumentPreview = () => {
  if (!floatPopup.value.knowledgeId) return;
  docPreviewDrawer.value = {
    visible: true,
    knowledgeId: floatPopup.value.knowledgeId,
    knowledgeTitle: floatPopup.value.knowledgeTitle,
    knowledgeBaseName: floatPopup.value.knowledgeBaseName,
    tagName: floatPopup.value.tagName,
    fileType: floatPopup.value.fileType,
  };
...
const syncFloatPopupFromCache = (chunkId: string, state: KbTooltipState) => {
  ...
  // 只有非空值才覆盖，保留 hover 时已设置的文档名
  if (state.knowledgeTitle) floatPopup.value.knowledgeTitle = state.knowledgeTitle;
  if (state.knowledgeBaseName !== undefined) floatPopup.value.knowledgeBaseName = state.knowledgeBaseName;
  if (state.tagName !== undefined) floatPopup.value.tagName = state.tagName;
  if (state.knowledgeId !== undefined) floatPopup.value.knowledgeId = state.knowledgeId;
  if (state.fileType !== undefined) floatPopup.value.fileType = state.fileType;
};
...
const loadChunkDetails = async (chunkId: string) => {
  ...
  try {
    ...
    if (content) {
      ...
      setKbCacheState(chunkId, {
        loading: false,
        html,
        knowledgeTitle: data?.knowledge_title || '',
...
.kb-float-popup .tip-doc-kbname {
  color: var(--td-brand-color);
}

.kb-float-popup .tip-doc-sep {
  color: var(--td-text-color-placeholder);
}

.kb-float-popup .tip-doc-tag {
...
.kb-float-popup .tip-preview-btn {
  flex-shrink: 0;
  display: flex;
  align-items: center;
