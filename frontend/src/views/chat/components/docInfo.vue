<template>
    <div class="refer" v-if="session.knowledge_references && session.knowledge_references.length">
        ...
        <div class="refer_box" v-show="showReferBox">
            ...
            <div v-for="(group, gIdx) in groupedKnowledgeRefs" :key="'grp-' + gIdx" class="doc-group">
                <div class="doc-group-header" @click="toggleGroup(group.key)">
                    ...
                    <div class="doc-group-actions" @click.stop>
                        <t-tooltip v-if="group.knowledgeId" :content="$t('chat.previewDocument')">
                            <span class="doc-group-navigate" @click="openPreview(group)">
                ...
                <div class="doc-group-chunks" v-show="expandedGroups[group.key]">
                    <div v-for="(chunk, cIdx) in group.chunks" :key="'chunk-' + cIdx" class="doc-chunk-item">
                        <t-popup overlayClassName="refer-to-layer" placement="bottom-left" width="400" :showArrow="false" trigger="click">
                            <template #content>
                                <ContentPopup
                                    :content="safeProcessContent(chunk.content)"
                                    :is-html="true"
                                    :knowledge-id="group.knowledgeId"
                                    :knowledge-title="group.title"
                                    :knowledge-base-name="group.knowledgeBaseName"
        ...
        <DocumentPreviewDrawer
            v-model:visible="previewDrawer.visible"
            :knowledgeId="previewDrawer.knowledgeId"
            :knowledgeTitle="previewDrawer.knowledgeTitle"
            :knowledgeBaseName="previewDrawer.knowledgeBaseName"
        />
    </div>
</template>
<script setup>
