...
type ChunkHandler struct {
	...
	tagRepository     interfaces.KnowledgeTagRepository
	kbShareService    interfaces.KBShareService
	agentShareService interfaces.AgentShareService
}

// NewChunkHandler creates a new chunk handler
func NewChunkHandler(service interfaces.ChunkService, kgService interfaces.KnowledgeService, kbService interfaces.KnowledgeBaseService, tagRepository interfaces.KnowledgeTagRepository, kbShareService interfaces.KBShareService, agentShareService interfaces.AgentShareService) *ChunkHandler {
	return &ChunkHandler{service: service, kgService: kgService, kbService: kbService, tagRepository: tagRepository, kbShareService: kbShareService, agentShareService: agentShareService}
}
...
func (h *ChunkHandler) GetChunkByIDOnly(c *gin.Context) {
	...
	// 聚合文档名、知识库名、标签名
	var knowledgeTitle, knowledgeBaseName, tagName, fileType string
	knowledge, kgErr := h.kgService.GetKnowledgeByIDOnly(ctx, chunk.KnowledgeID)
	if kgErr == nil && knowledge != nil {
		knowledgeTitle = knowledge.Title
		if knowledge.Title == "" {
			knowledgeTitle = knowledge.FileName
		}
		fileType = knowledge.FileType
		// 获取知识库名
		if h.kbService != nil && knowledge.KnowledgeBaseID != "" {
		...
		// 获取标签名
		if h.tagRepository != nil && knowledge.TagID != "" {
			tenantID := knowledge.TenantID
			tag, tagErr := h.tagRepository.GetByID(ctx, tenantID, knowledge.TagID)
