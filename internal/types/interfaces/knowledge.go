...
type KnowledgeService interface {
	...
	GetKnowledgeBatch(ctx context.Context, ids []string) ([]*types.Knowledge, error)
	// GetKnowledgeBatchWithSharedAccess retrieves knowledge by IDs including items from shared KBs the user has access to.
	GetKnowledgeBatchWithSharedAccess(ctx context.Context, ids []string) ([]*types.Knowledge, error)
	...
	// When tagID is non-empty, results are filtered by tag ID on FAQ chunks.
	...
	UpdateFAQEntryTagBatch(ctx context.Context, kbID string, updates map[int64]*string) error
	...
	// SearchKnowledgeForScopes searches knowledge within the given kb_id scopes.
...
type KnowledgeRepository interface {
	...
	CountKnowledgeByKnowledgeBaseID(ctx context.Context, kbID string) (int64, error)
	CountKnowledgeByStatus(ctx context.Context, kbID string, parseStatuses []string) (int64, error)
	SearchKnowledge(ctx context.Context, keyword string, offset, limit int, fileTypes []string) ([]*types.Knowledge, bool, error)
	...
	ListIDsByTagID(ctx context.Context, kbID, tagID string) ([]string, error)
}
